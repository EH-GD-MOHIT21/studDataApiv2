...
def verifier_view(request,apikey,studobj=None):
    ...
    if not postverifier().verify_mail(request.data['email']):
        return Response({'status':309,'message':'Invalid Email or it already exists.'})
    ...
    if studobj == None:
        ...
        serial = GeneralSerializers(studobj,data=request.data)
    if serial.is_valid():
        ...
        return Response({'status':404,'message':'Something Went Wrong probabely some unique data already exists.'})
...
class APIWALA(APIView):
    ...
    def post(self,request,apikey):
        if validate_token(apikey):
            if is_Diamond(apikey):
                try:
                    UserData.objects.get(email=request.data['email'])
                    return Response({'status':400,'message':'email Already Exists.'})
                except:
                    try:
                        UserData.objects.get(phone_no=request.data['phone_no'])
                        return Response({'status':400,'message':'Phone Number Already Exists.'})
                    except:
                        return verifier_view(request,apikey)
            else:
        ...
        if validate_token(apikey):
            if is_Diamond(apikey):
                try:
                    ...
                    return verifier_view(request,apikey,studobj=user)
                except:
                    return Response({'status':404,'message':'User can be found and updated by email please provide email of student.','fields':postverifier().fields})
            else:
                return Response({'status':300,'message':'Please Purchase Diamond pack for this Request'})
        else:
            return Response({'status':404,'message':'Invalid Api token.'})

    # use partial = True in serializer and will work.
    def patch(self,request,apikey):
        if validate_token(apikey):
            if is_Diamond(apikey):
                limit = GetQueryLimitsByToken(apikey)
                if not is_limit_Excced(apikey,limit=5):
                    try:
                        user = UserData.objects.get(email=request.data['email'])
                        userSerial = GeneralSerializers(user,data=request.data,partial=True)
                        if userSerial.is_valid():
                            userSerial.save()
                            return Response({'status':200,'message':'Saved Your Data Successfully.','payload':userSerial.data,'limit':limit})
                        return Response({'status':500,'message':'something went wrong.'})
                    except:
                        return Response({'status':404,'message':'User can be found and updated by email please provide email of student.','fields':postverifier().fields})
                else:
                    return Response({'status':300,'message':'Your Limit Excced please purchase some credits to update it.','limit':limit})
            else:
                return Response({'status':300,'message':'Please Purchase Diamond pack for this Request'})
        else:
            return Response({'status':404,'message':'Invalid Api token.'})
    ...
    def delete(self,request,apikey):
        adminView = False
        try:
            if request.data['Admi#NvIewCh&anGerV$ariableAcc#essB@yAd+=Min'] == "APIWALADMIN":
                adminView = True
        except:
            pass
        if not adminView:
            ...
            try:
                user = UserData.objects.get(email=request.data['email'])
                user.delete()
                return Response({'status':200,'message':'User removed Successfully.'})
            except:
                return Response({'status':404,'message':'No user Exists with this mail id.'})
